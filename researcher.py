...
class Researcher(object):
    ...
    def get_random_dataset(self):
        ...
        req = s.get("http://192.168.149.168/api/v1/dataset/", headers=headers)
    ...
    def run(self):
        ...
        print dataset["lowest_ts"], upper_bound
        
        s = requests.Session()
        headers = {'content-type': 'application/json'}
        #read interval
        req = s.get("http://192.168.149.168%s?upper_time=%s" % (dataset["resource_uri"], str(upper_bound)), headers=headers)
